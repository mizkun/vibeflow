...
### 1. 状態確認
`.vibe/project_state.yaml` を読み込み、現在の phase を確認する。
- `quickfix` の場合: 「既に Quick Fix モード中です」と表示

### 2. コンテキスト読み込み
1. `.vibe/context/STATUS.md` を読み込み、プロジェクトの現状を把握
2. `.vibe/spec/` で構造化 spec を確認
3. 対象領域のソースコードを確認

### 3. モード開始
1. **Phase 切り替え**: `phase: quickfix` に更新
2. **Role 切り替え**: `current_role: "Iris"` に更新（Iris が直接対応）
...
- **src/ への書き込み可**: coding agent と同じ Write scope
- **plan.md / vision.md / .vibe/spec/ への書き込み不可**

#### 終了条件
ユーザーが満足を表明したら（「コミットして」「OK これで」など）:
1. 変更内容をまとめた atomic commit を作成
2. project_state.yaml を復帰:
   - `phase: development`
   - `current_role: "Iris"`
