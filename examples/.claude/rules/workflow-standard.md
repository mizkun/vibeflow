...
- **UI task ではない**
- **構造化 spec (`.vibe/spec/`) を変更していない**

### 人間チェック必要 (needs_human)
- **構造化 spec の変更**（`.vibe/spec/` の Story/Contract を変更した PR は
  必ず Human Checkpoint を通す。spec の As-Is → To-Be 差分をユーザーに提示し、
  承認を得てから `.vibe/checkpoints/<issue>-qa-approved`（内容 `approved`）を
  作成する。qa:auto では自動承認できない）
...
> spec 変更時の needs_human は honor-system ではなく `validate_step7a.py`
> hook（Spec Gate）で強制される。spec を変更した PR は、人間承認の
> checkpoint がない限り `gh pr create` がブロックされる。
